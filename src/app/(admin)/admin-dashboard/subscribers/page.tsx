...
export default function AdminSubscribersPage() {
  ...
  const closeDrawer = () => { setSelectedSub(null); setIsEditing(false); };
  const startEdit = () => {
    if (!selectedSub) return;
    setIsEditing(true);
    setEditForm({ status: selectedSub.status, dietaryPreferences: [...selectedSub.dietaryPreferences], schedule: selectedSub.schedule.map(s => ({ ...s })), dietaryNote: selectedSub.dietaryNote || '' });
  };
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-2">
                      <div className="flex flex-wrap gap-1 max-w-[150px]">
                        {sub.dietaryPreferences.length > 0 ? sub.dietaryPreferences.map((p, i) => (
              ...
              <div className="flex items-center gap-3">
                {!isEditing ? (
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { alert('Subscriber updated! (Mock — will persist with BE)'); setIsEditing(false); }} className="px-4 py-2 bg-[#FF8C00] text-white rounded-xl text-sm font-bold cursor-pointer hover:opacity-90 active:scale-95 transition-all shadow-lg shadow-orange-500/20">Save</button>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {(isEditing ? editForm.schedule : selectedSub.schedule).map((item, i) => (
